...
#load sol
circuit.ry(2*np.arccos(sol[0]),qubits[-1])
...
#Only count instances when first qubit is 1
...
print("Y COUNT: "+str(y_count))
print("K: "+str(k))
print("X VALUE: "+str(x_value))
print("Y VALUE: "+str(y_value))

final_value = np.sqrt((x_value**2)+(y_value**2))
print("VECTOR NORM: "+str(final_value))

total = 0
x_count = 0
y_count = 0
for measure in counts:
    if (measure[-1]=='1' and (measure[1]=='1' or measure[2]=='1')):
        #print("ISNT 0: "+measure)
        total+=counts[measure]

print("TOTAL NOT 0: "+str(total))
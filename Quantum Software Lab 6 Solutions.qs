...
namespace Lab6 {
    ...
    operation Exercise3 (
        ...
        //Message matches encryption key flip the target when have the correction encryption key 

        Exercise1(originalMessage,candidateEncryptionKey);

        for i in 0..Length(encryptedMessage){
        ...
        // let register = candidateEncryptionKey;
        

        // Controlled Z();

        // for i in 0..Length(register)-1{
        //     if(ResultAsBool(M(register[i]))==encryptedMessage[i]){
        //         X(ancilla[i]);
        //     }
        // }
        // Controlled Z(ancilla, target);
        // Adjoint Exercise1(originalMessage,candidateEncryptionKey);

...
namespace Lab9 {
    ...
    operation Exercise1 (
        ...
        for i in 0..Length(input)-1{
            let n = Length(input)-1-i;
            let c = ExpModI(a, 2^(n),b);


            Controlled MultiplyByModularInteger([input[i]], (c, b, LittleEndian(output)));
    ...
    operation Exercise2 (
        ...
        let b = numberToFactor;
        let a = guess;
        let m = Ceiling(Log10(IntAsDouble(b)+1.0)/Log10(2.0));
        let n = 2*m;
        ...
        Exercise1(a, b,input,output);

        Adjoint QFT(BigEndian(input));

        let measured = MeasureInteger(BigEndianAsLittleEndian(BigEndian(input)));

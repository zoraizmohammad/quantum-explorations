...
def exercise_2(circuit, register, target):
    ...
    ancilla_register = QuantumRegister(len(register)-1)
    ...
    circuit.ccx(register[0],register[1],ancilla_register[0])

    for i in range(1,len(ancilla_register)):
        circuit.ccx(register[i+1], ancilla_register[i-1], ancilla_register[i])

    circuit.cz(ancilla_register[-1], target)
...
def exercise_5(oracle, number_of_qubits):
    ...
    """
    iterations = round(pow(2.0, double(number_of_qubits)/2.0))

    input = QuantumRegister(number_of_qubits)
    output = QuantumRegister(1)
    measurement = ClassicalRegister(number_of_qubits)
    circuit = QuantumRegister(input, output, measurement)

    circuit.h(input)
    circuit.x(output)

    for i in range(1, iterations+1)
        exercise_4(circuit, oracle, input, output)
    
    simulator = Aer.get_backhand('aser_simulator')
    simulation = execute(circuit, simulator, shots=1)
    result = simulation.result(circuit)

    counts = result.get_counts(circuit)

    for(measured_state) in counts.items():
